...
Two commands work everywhere: `/link` and `/help`. Every other group is installed into
a server only while that server is [connected to an organization](./connect-server), and
is removed again on disconnect.
